            ...
            case 'grupo-familiar':
                return (
                    <div className="space-y-6 animate-in fade-in duration-500 text-[#1C75BB]">
                        ...
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                            <h2 className="text-xs font-black uppercase tracking-widest mb-6 opacity-70">Grupo Familiar en Sistema</h2>
                            <div className="overflow-hidden border border-gray-100 rounded-2xl">
                                <table className="w-full text-left">
                                    <thead className="bg-gray-50 text-xs font-black uppercase text-[#1C75BB]">
                                        <tr>
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {credenciales.map((m, i) => (
                                            <tr key={i} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-6 py-5 text-center font-mono text-sm text-[#1C75BB]">{m.nroCarnet}</td>
                                                <td className="px-6 py-5 text-center text-xs font-black uppercase text-green-600 tracking-widest">Activo</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                    ...
                    <div className="space-y-6 animate-in fade-in duration-500">
                        {cartillaView === 'filtros' ? (
                            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-10 text-[#1C75BB]">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-[#1C75BB]">
                                    <div className="space-y-2">
                                        <label className="text-xs font-black uppercase opacity-70 ml-1">Tipo de Atención *</label>
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-black uppercase opacity-70 ml-1">Razón Social o Nombre</label>
                                    ...
                                    {(filtrosCartilla.tipoCartilla === "1" || filtrosCartilla.tipoCartilla === "7") && (
                                        <div className="md:col-span-2 space-y-2">
                                            <label className="text-xs font-black uppercase opacity-70 ml-1">Especialidad *</label>
                                            <select className="w-full bg-slate-50 border-none rounded-xl p-4 font-bold outline-none focus:ring-2 focus:ring-[#00AEEF] appearance-none"
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-black uppercase opacity-70 ml-1">Provincia</label>
                                        <select className="w-full bg-slate-50 border-none rounded-xl p-4 font-bold outline-none focus:ring-2 focus:ring-[#00AEEF] appearance-none"
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-black uppercase opacity-70 ml-1">Localidad</label>
                                        <select disabled={!filtrosCartilla.provinciaId} className="w-full bg-slate-50 border-none rounded-xl p-4 font-bold outline-none focus:ring-2 focus:ring-[#00AEEF] appearance-none disabled:opacity-60"
                            ...
                            <div className="space-y-6">
                                <button onClick={() => setCartillaView('filtros')} className="flex items-center gap-2 font-black text-xs uppercase tracking-widest text-[#1C75BB] hover:underline mb-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {resultadosCartilla.map((p, idx) => (
                                        <div key={idx} className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm text-[#1C75BB] hover:shadow-md transition-all">
                                            ...
                                            {p.especialidad && <p className="text-[#00AEEF] text-xs font-bold uppercase tracking-widest mb-4 italic">{p.especialidad}</p>}
                                            <div className="space-y-3 pt-4 border-t border-gray-50 opacity-80">
                                                <div className="flex items-start gap-3"><MapPin size={16} className="shrink-0 mt-0.5 opacity-70" /><p className="text-sm font-bold leading-snug">{p.direccion}, {p.localidad}</p></div>
                    ...
                    <div className="space-y-6 animate-in fade-in duration-500 pb-10 text-[#1C75BB]">
                        ...
                        <div className="bg-white rounded-3xl p-10 border border-gray-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8">
                            <div className="flex items-center gap-6">
                                ...
                                <div><p className="text-xs font-black uppercase opacity-70 mb-1 tracking-widest">Saldo deudor total</p><h3 className="text-4xl font-black text-gray-900 tracking-tighter">${saldo.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</h3></div>
                            </div>
                            <button className="px-10 py-4 bg-[#1C75BB] text-white font-black rounded-xl shadow-lg hover:bg-[#00AEEF] transition-all uppercase tracking-widest text-xs">Pagar Cuotas Online</button>
                        </div>

                        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                            <table className="w-full text-left">
                                <thead className="bg-gray-50 text-sm font-black uppercase tracking-widest text-[#1C75BB]">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {movimientos.slice(0, visibleCount).map((m) => (
                                        <tr key={m.id} className="hover:bg-slate-50 transition-colors">
                                            <td className="px-8 py-6 font-bold text-sm text-center">{new Date(m.fecha_movimiento).toLocaleDateString('es-AR')}</td>
                                            <td className="px-8 py-6"><p className="text-xs font-black opacity-60 uppercase leading-none">{m.tipo_comprobante}</p><p className="font-bold text-sm tracking-tight">{m.numero_comprobante}</p></td>
                            ...
                            {visibleCount < movimientos.length && (
                                <div className="p-8 text-center bg-gray-50/50">
                                    <button onClick={handleLoadMore} className="px-8 py-3 bg-white border border-gray-200 rounded-xl font-bold text-sm uppercase text-[#1C75BB] shadow-sm hover:bg-slate-100 transition-all">
                        ...
                        <div className="bg-white rounded-3xl p-10 border border-gray-100 shadow-sm">
                            ...
                            {emailStep === 1 ? (
                                <form onSubmit={handleEmailRequest} className="space-y-6">
                                    <p className="text-[#1C75BB] text-xs font-bold mb-4 leading-relaxed">Ingresá el nuevo email. Te enviaremos un código para confirmar el cambio.</p>
                                    ...
                                    <button className="px-10 bg-white text-[#1C75BB] font-black py-4 rounded-xl border border-gray-200 uppercase tracking-widest text-xs shadow-sm">Solicitar Cambio</button>
                                </form>
                            ) : (
                                <form onSubmit={async (e) => {
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <button type="button" onClick={() => setEmailStep(1)} className="px-6 text-gray-500 font-bold uppercase text-xs tracking-widest">Cancelar</button>
                                    </div>
                                </form>
                            )}
                        </div>
            ...
            <header className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
                <div className="max-w-full mx-auto px-6 lg:px-16 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        ...
                        <span className="hidden md:block text-xs font-black uppercase tracking-[0.3em] border-l pl-6 border-gray-100 text-gray-400">Portal Afiliados</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <button onClick={onGoHome} className="flex items-center gap-2 px-5 py-2 bg-slate-100 text-[#1C75BB] rounded-2xl font-black text-xs transition-all shadow-sm hover:bg-[#00AEEF]/10 hover:text-[#00AEEF] uppercase tracking-widest active:scale-95">
                            <Home size={14} /> Inicio
                        </button>
                        <button onClick={onLogout} className="flex items-center gap-2 px-5 py-2 bg-slate-100 text-[#1C75BB] rounded-2xl font-black text-xs transition-all shadow-sm hover:bg-red-50 hover:text-red-500 uppercase tracking-widest active:scale-95">
                            ...
                            {menuItems.map((item) => (
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between px-6 py-4 rounded-2xl font-black transition-all ${activeSection === item.id
                                        ...
                                        : 'text-[#1C75BB] hover:bg-slate-50 hover:text-[#1C75BB]'
                                        }`}
                                >
